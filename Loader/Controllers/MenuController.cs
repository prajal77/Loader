...
using DataEntities = Loader.Repository;
...
namespace Loader.Controllers
{
    ...
    public class MenuController : Controller
    {
        ...
        ApplicationDbContext myContext = new DataEntities.GenericUnitOfWork().GetContext();
        ...
        public async Task<ActionResult> Index()
        {
            try
            {
                ...
                if (Request.IsAjaxRequest())
                {
                    ...
                    //SIGNALR SECTION
                    var abcd = menuService.GetAllOfParent(0);
                    var collection = myContext.Menu.Where(x=>x.PMenuId==0).AsQueryable();
                     ViewBag.NotifierEntity = myContext.GetNotifierEntity<Menu>(collection).ToJson();
                    return View(await Task.Run(() => abcd.ToList()));
        ...
        public async Task<ActionResult> _Details(int parentId, bool allowSelectGroup, int ViewType = 1)
        {
            ViewBag.Address = menuService.GetAddress(parentId);
            ViewBag.ParentMenuId = parentId;
            ViewBag.ViewType = ViewType;
            var collection = myContext.Menu.Where(x => x.PMenuId.Equals(parentId)).AsQueryable();
            //var collection = myContext.Menu.Where(x => x.PMenuId == 0).AsQueryable();
            ViewBag.NotifierEntity = myContext.GetNotifierEntity<Menu>(collection).ToJson();
            return PartialView(await Task.Run(() => collection.ToList()));
