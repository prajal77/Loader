    ...
    public class MenuTemplateController : Controller
    {
        ...
        public ActionResult Create(int MenuTemplateId = 0)
        {
            ...
            if (Request.IsAjaxRequest())
            {
                ...
                {
                    ViewBag.ActiveText = new Loader.Repository.GenericUnitOfWork().Repository<Designation>().GetAll().Where(x => x.DesignationId == MenuTemplateDTO.DesignationId).Select(x => x.DGName).SingleOrDefault();
                }
                else
                {
