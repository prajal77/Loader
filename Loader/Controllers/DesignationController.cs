...
namespace Loader.Controllers
{
    ...
    public class DesignationController : Controller
    {
        ...
        [HttpGet]
        public ActionResult Create(string activeText, int activeId, int DesignationId = 0)
        {
            if (Request.IsAjaxRequest())
            {
                ...
                if (DesignationId != 0)
                {
                    ...
                    DesignationDTO = new Loader.Repository.GenericUnitOfWork().Repository<Designation>().GetSingle(x => x.DesignationId == DesignationId);
        ...
        public ActionResult Create(Designation Designation, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    {
                        DesignationService.Save(Designation);
                        var parentNode = new Loader.Repository.GenericUnitOfWork().Repository<Designation>().GetSingle(x => x.DesignationId == Designation.PDGId);
                        if (parentNode == null)
                        {
                            return RedirectToAction("Create", new { activeText = "Root", activeId =0});
                        }
                        ...
                        return RedirectToAction("Create", new { activeText = parentNode.DGName, activeId = parentNode.DesignationId });
                    }
