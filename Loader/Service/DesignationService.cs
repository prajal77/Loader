...
namespace Loader.Service
{
    public class DesignationService
    {
        ...
        {
            var dValue = uow.Repository<ParamValue>().GetAll().Where(x => x.PId == (int)Loader.Helper.EnumValue.Parameter.DesignationInHeirarchy).Select(x => x.PValue).SingleOrDefault();
            return dValue;
        }

        public Designation GetSingle(int DesignationId)
        {
            Designation Designation = uow.Repository<Designation>().GetSingle(c => c.DesignationId == DesignationId);
            return Designation;
        }
        ...
        public void Save(Designation Designation)
        {
            ...
            int checkExists = editUOW.Repository<Designation>().GetAll().Where(x => x.DesignationId != Designation.DesignationId && x.DGName == Designation.DGName && x.PDGId == Designation.PDGId).Count();
            if (checkExists > 0)
            {
        ...
        public ViewModel.TreeView GetDesignationGroupTree(string filter = "")
        {
            ...
            list.Add(new Designation { DesignationId = 0, DGName = "Root", PDGId = -1 });
        ...
        {
            List<Designation> list = uow.Repository<Designation>().GetAll().Where(x => x.DesignationId != DesignationIdExpect).ToList();
            list.Add(new Designation { DesignationId = 0,  DGName = "Root", PDGId = -1 });
