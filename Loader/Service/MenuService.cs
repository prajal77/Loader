...
namespace Loader.Service
{
    public class MenuService
    {
        ...
        public IQueryable<Menu> GetAllOfParent(int parentId)
        {
            return uow.Repository<Menu>().GetAllAsQuerable().Where(x => x.PMenuId == parentId).AsQueryable();
        }
        ...
        {
            LayoutMenuMainService menumainService = new Loader.Service.LayoutMenuMainService();
            return menumainService.GetLayoutMenuGroupTree(filter);
            ...
            //var menuListFinal = from m in uow.Repository<Menu>().GetAll() join p in menuList on m.MenuId equals p select new Menu { MenuId = m.MenuId, MenuCaption = m.MenuCaption, IsGroup = m.IsGroup, PMenuId = m.PMenuId, IsEnable = m.IsEnable, Acton = m.Acton, Controler = m.Controler, Image = m.Image, IsContextMenu = m.IsContextMenu };
            //List<Menu> list = menuListFinal.ToList();
            //if (userId == 1)
            //{
            //    list = uow.Repository<Models.Menu>().GetAll().ToList();
            //}
