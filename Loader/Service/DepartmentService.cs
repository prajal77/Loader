...
namespace Loader.Service
{
    public class DepartmentService
    {
        ...
        {
            var dValue = uow.Repository<ParamValue>().GetSingle(x => x.PId == (int)EnumValue.Parameter.DepartmentinHeirarchy).PValue;
            return dValue;
        }

        public Department GetSingle(int DepartmentId)
        {
            Department Department = uow.Repository<Department>().GetSingle(c => c.DepartmentId == DepartmentId);
            return Department;
        }
        ...
        public void Save(Department Department)
        {
            ...
            int checkExists = editUOW.Repository<Department>().GetAll().Where(x => x.DepartmentId != Department.DepartmentId && x.DeptName == Department.DeptName && x.PDeptId == Department.PDeptId).Count();
            if (checkExists > 0)
            {
        ...
        public ViewModel.TreeView GetDepartmentGroupTree(string filter = "")
        {
            ...
            list.Add(new Department { DepartmentId = 0, DeptName = "Root", PDeptId = -1 });
        ...
        {
            List<Department> list = uow.Repository<Department>().GetAll().Where(x => x.DepartmentId != DepartmentIdExpect).ToList();
            list.Add(new Department { DepartmentId = 0, DeptName = "Root", PDeptId = -1 });
